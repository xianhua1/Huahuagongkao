...
const dir = 'C:\\Users\\admin\\DSH\\tools\\edge-mark5'
fs.rmSync(dir, { recursive: true, force: true })
const edge = spawn('C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe', [
  '--headless=new', '--remote-debugging-port=9243', `--user-data-dir=${dir}`,
...
for (let i = 0; i < 40; i++) {
  try {
    const l = await (await fetch('http://127.0.0.1:9243/json')).json()
...
const ev = async e => {
  try {
    ...
    return r && r.result ? r.result.value : 'EVAL_FAIL'
...
await sleep(4000)
console.log('材料容器数:', await ev(`document.querySelectorAll('.sl-material').length`))
console.log('展开full数:', await ev(`document.querySelectorAll('.sl-material-full').length`))
console.log('预览数:', await ev(`document.querySelectorAll('.sl-material-preview').length`))
console.log('标注提示:', await ev(`(() => { const t = document.querySelector('.sl-mark-hint'); return t ? t.innerText : 'none' })()`))
ws.close()
edge.kill()
process.exit(0)
