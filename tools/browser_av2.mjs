...
const dir = 'C:\\Users\\admin\\DSH\\tools\\edge-av2'
fs.rmSync(dir, { recursive: true, force: true })
const edge = spawn('C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe', [
  '--headless=new', '--remote-debugging-port=9246', `--user-data-dir=${dir}`,
...
for (let i = 0; i < 40; i++) {
  try {
    const l = await (await fetch('http://127.0.0.1:9246/json')).json()
...
await sleep(7000)
await send('Page.navigate', { url: 'http://127.0.0.1:8090/system/user/profile' })
await sleep(5000)
console.log('URL:', await ev('location.href'))
console.log('user-info-head 存在:', await ev(`!!document.querySelector('.user-info-head')`))
console.log('页面文本片段:', (await ev(`document.body.innerText.slice(0, 300)`)).replace(/\n+/g, ' | ').slice(0, 200))
ws.close()
edge.kill()
process.exit(0)
